// Фоновый поток нейросети: загрузка модели, отпечатки фраз и сравнение не тормозят видео.
...
let session = null;
let items = []; // { id, lang, label, T, feats: Float32Array, preset? }
...
async function loadModel() {
  ...
  for (;;) {
    ...
    if (pct !== last) self.postMessage({ type: 'progress', value: (last = pct) });
  }
...
function resample(frames, times) {
  if (!times || times.length !== frames.length || frames.length < 2) return frames;
  const out = [];
  let j = 0;
...
async function embed(bytes, times) {
  const n = bytes.length / (CROP * CROP);
  const frames = Array.from({ length: n }, (_, i) => bytes.subarray(i * CROP * CROP, (i + 1) * CROP * CROP));
  const clip = resample(frames, times);
  const T = clip.length;
...
const commands = {
  ...
  async enroll({ lang, label, bytes, times }) {
    const { T, feats } = await embed(bytes, times);
    const item = { lang, label, T, feats };
    item.id = await dbPut(item);
    items.push(item);
    cache = null;
    return items.filter((it) => it.lang === lang && it.label === label && !it.preset).length;
  },

  async classify({ lang, bytes, times }) {
...
// Команды выполняются по очереди: сессия ONNX не любит параллельных запусков.
let queue = Promise.resolve();
self.onmessage = (e) => {
  const { id, cmd, args } = e.data;
  queue = queue.then(async () => {
    try {
      self.postMessage({ id, ok: true, result: await commands[cmd](args || {}) });
    } catch (err) {
